...
import ast
import pandas as pd
datadir = os.path.join(os.path.dirname(__file__), "database/kaggle")
...
def load_dataset(save=False) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if save:
        combined_df.to_csv(os.path.join(datadir, "combined.csv"), index=False)

    return combined_df

if __name__ == "__main__":
    download_data()
    load_dataset(save=True)